...
--CREATE TABLE C_SCHEMA.C_LOG_TABLE(
--ID NUMBER,
--MSG VARCHAR2(500),
--LOAD_DATE DATE
--);
MY_SQL := 'SELECT (MAX(ID)+1) FROM ( SELECT MAX(ID) ID FROM '|| C_SCHEMA ||'.'|| C_LOG_TABLE || ' UNION SELECT 0 FROM DUAL )';
EXECUTE IMMEDIATE MY_SQL INTO MY_ID;
MY_SQL := 'INSERT INTO '|| C_SCHEMA ||'.'|| C_LOG_TABLE || '(ID,MSG,LOAD_DATE) VALUES('|| MY_ID ||','''|| MSG||''',SYSDATE)';
